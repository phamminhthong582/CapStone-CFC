...
public class ChatGptService
{
    ...
    public ChatGptService(IConfiguration configuration, IFlowerRepository flowerRepository, IFlowerBasketRepository flowerBasketRepository)
    {
    ...
    public async Task<string> GenerateTextAsync(string userMessage)
    {
        ...
        string prompt = $"You are a chatbot assisting with a flower ordering system. The user asks: '{userMessage}'. Here is some information: {flowerInfo ?? "No flower info available"} {flowerBasketInfo ?? "No flower basket info available"}";
        var requestBody = new
        {
            model = "gpt-3.5-turbo", 
            messages = new[]
